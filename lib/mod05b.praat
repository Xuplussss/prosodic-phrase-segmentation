...
###########################################################################
##### MODULE 5 ############################################################
###### PPh boundary prediction (on raw speech)							  #
###########################################################################
clearinfo
form Parameters
	text directory 
...
for i_int from 1 to n_pph
	...
	check1 = Get interval boundary from time: 5, start_pph
	check_lab$ = Get head of interval: 2, i_int
	if start_pph != 0 && check1 = 0 && check_lab$ = ""
		Insert boundary: 5, start_pph
	endif
	...
	if check_lab$ = ""
		...
		if time_min2 != 0 && dur_pph > 4
			selectObject: text$
			Insert boundary: 5, time_min2
		endif
	endif
endfor
## Annotate features of the predicted PPh intervals
count = 0
n_pph2 = Get number of intervals: 5

for i_pph to n_pph2
	count += 1
	selectObject: text$
	start = Get starting point: 5, i_pph
	end = Get end point: 5, i_pph
	time = end - start
	time$ = fixed$ (time, 2)
	...
	count$ = string$(count)
	selectObject: text$
	Set interval head text: 5, i_pph, count$
	Insert feature to interval: 5, i_pph, "t_min", t_min$
	Insert feature to interval: 5, i_pph, "t_maxf0", t_maxf0$
	Insert feature to interval: 5, i_pph, "z_int", z_int$
	Insert feature to interval: 5, i_pph, "z_f0", z_f0$
	Insert feature to interval: 5, i_pph, "dur", time$
	Insert feature to interval: 5, i_pph, "dur_all", rel_dur_all$ + "%"		
endfor
